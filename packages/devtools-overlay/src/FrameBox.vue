...
<template>
  <div v-show="state.open" ref="container" class="devtools-frame" :class="viewModeClass">
    <!-- Handlers -->
    <div
      v-show="state.position !== 'top'" class="devtools-resize devtools-resize--horizontal" :style="{ top: 0 }"
      @mousedown.prevent="() => isResizing = { top: true }"
    />
    <div
      v-show="state.position !== 'bottom'" class="devtools-resize devtools-resize--horizontal"
      :style="{ bottom: 0 }" @mousedown.prevent="() => isResizing = { bottom: true }"
    />
    <div
      v-show="state.position !== 'left'" class="devtools-resize devtools-resize--vertical" :style="{ left: 0 }"
      @mousedown.prevent="() => isResizing = { left: true }"
    />
    <div
      v-show="state.position !== 'right'" class="devtools-resize devtools-resize--vertical"
      :style="{ right: 0 }" @mousedown.prevent="() => isResizing = { right: true }"
    />
    <div
      v-show="state.position !== 'top' && state.position !== 'left'"
      class="devtools-resize devtools-resize-corner" :style="{ top: 0, left: 0, cursor: 'nwse-resize' }"
      @mousedown.prevent="() => isResizing = { top: true, left: true }"
    />
    <div
      v-show="state.position !== 'top' && state.position !== 'right'"
      class="devtools-resize devtools-resize-corner" :style="{ top: 0, right: 0, cursor: 'nesw-resize' }"
      @mousedown.prevent="() => isResizing = { top: true, right: true }"
    />
    <div
      v-show="state.position !== 'bottom' && state.position !== 'left'"
      class="devtools-resize devtools-resize-corner" :style="{ bottom: 0, left: 0, cursor: 'nesw-resize' }"
      @mousedown.prevent="() => isResizing = { bottom: true, left: true }"
    />
    <div
      v-show="state.position !== 'bottom' && state.position !== 'right'"
      class="devtools-resize devtools-resize-corner" :style="{ bottom: 0, right: 0, cursor: 'nwse-resize' }"
